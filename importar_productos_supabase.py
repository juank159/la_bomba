#!/usr/bin/env python3
"""
🚀 IMPORTADOR INTERACTIVO DE PRODUCTOS PARA SUPABASE
=====================================================
Script visual e interactivo para importar productos desde CSV a Supabase.
...
def create_backup(logger: Logger):
    ...
    try:
        ...
        # Exportar datos de productos activos
        cursor.execute("""
            SELECT description, barcode, "isActive", precioa, preciob, precioc, costo, iva
            FROM products
            WHERE "isActive" = true
            ORDER BY "createdAt"
        """)
        ...
        with open(backup_file, 'w', encoding='utf-8') as f:
            f.write(f"-- Backup de productos - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"-- Total productos: {len(products)}\n\n")
            f.write("BEGIN;\n\n")

            for product in products:
                description, barcode, isActive, precioA, precioB, precioC, costo, iva = product
                description = description.replace("'", "''")
                barcode = barcode.replace("'", "''")

                f.write(f"INSERT INTO products (description, barcode, \"isActive\", precioa, preciob, precioc, costo, iva, \"createdAt\", \"updatedAt\")\n")
                f.write(f"VALUES ('{description}', '{barcode}', {isActive}, {precioA}, ")
                f.write(f"{precioB if precioB is not None else 'NULL'}, ")
                f.write(f"{precioC if precioC is not None else 'NULL'}, ")
                f.write(f"{costo if costo is not None else 'NULL'}, {iva}, NOW(), NOW());\n\n")
        ...
        print_success(f"Backup creado: {backup_file} ({len(products)} productos)")
...
def execute_sql(sql_file: str, logger: Logger):
    ...
    print_header("⚙️  EJECUTANDO SQL EN SUPABASE")
    logger.info("Ejecutando SQL en Supabase...")

    try:
        conn = get_db_connection()
        cursor = conn.cursor()

        print_info("Leyendo archivo SQL...")
        ...
        print_error(f"Error ejecutando SQL: {str(e)}")
...
def main():
    ...
    try:
        # Banner inicial
        print_header("🚀 IMPORTADOR DE PRODUCTOS PARA SUPABASE")
